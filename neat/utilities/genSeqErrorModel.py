...
#
#          genSeqErrorModel.py
#          Computes sequencing error model for gen_reads_runner.py
#
#         
...
def parse_file(input_file, real_q, off_q, max_reads, n_samp, plot_stuff):
    ...
    q_scores = np.arange(real_q)
    # print (len(init_q), len(init_q[0]))
    # print (len(prob_q), len(prob_q[1]), len(prob_q[1][0]))
    rng = np.random.default_rng()
    init_dist_by_pos = [DiscreteDistribution(q_scores, np.array(init_q[i]), rng) for i in range(len(init_q))]
    ...
    for i in range(1, len(init_q)):
        ...
        for j in range(len(init_q[0])):
            if np.sum(prob_q[i][j]) <= 0.:  # if we don't have sufficient data for a transition, use the previous qscore
                prob_dist_by_pos_by_prev_q[-1].append(DiscreteDistribution(np.array([q_scores[j]]), np.array([1]), rng,
                                                                           degenerate_val=q_scores[j]))
            else:
                prob_dist_by_pos_by_prev_q[-1].append(DiscreteDistribution(np.array(q_scores),
                                                                           np.array(prob_q[i][j]), rng))
