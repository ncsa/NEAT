...
def parse_beds(options: Options, reference_dict: _IndexedSeqFileDict, average_mutation_rate: float) -> list:
    """
    This single function parses the three possible bed file_list types for NEAT.
...
def parse_single_bed(input_bed: str,
    ...
    """
    Will parse a bed file_list, returning a dataframe of chromosomes that are found in the reference,
    ...
    :param input_bed: A bed file_list containing the regions of interest
    ...
    if input_bed:
            ...
            for line in f:
                if not line.startswith(('@', '#', "\n")):
                    ...
                    try:
                        ...
                        _LOG.error(f"Improperly formatted bed file_list line {line}")
                        raise
                    ...
                    try:
                        ...
                        try:
                            ...
                            _LOG.warning("Found chromosome in BED file_list that isn't in Reference file_list, skipping")
                            continue
        ...
        if in_ref_only:
            _LOG.warning(f'Warning: Reference contains sequences not found in BED file_list {input_bed}. '
                         f'These chromosomes will be omitted from the outputs.')
            _LOG.debug(f"In reference only regions: {in_ref_only}")

        if in_bed_only:
            _LOG.warning(f'BED file_list {input_bed} contains sequence names '
...
def fill_out_bed_dict(ref_dict: _IndexedSeqFileDict,
    ...
    """
    This parses the dict derived from the bed file_list and fills in any gaps, so it can be more easily cycled through
    later.
    ...
    :param ref_dict: The reference dictionary for the run. Needed to calulate max values. We shouldn't need to access the data it
        refers to.
    :param region_dict: A dict based on the input from the bed file_list, with keys being all the chronmosomes
    ...
    for contig in region_dict:
        ...
        # This is to prevent the coverage from being 2% of the target when no targeted bed file_list was included.
