...
class Read:
    ...
    def __init__(self,
        ...
        # Literal reference bases in reference_segment beyond the read's true genomic window,
        # fixed here because self.genomic_length is later resynced down for a short-insert
        # deletion (see _resync_short_insert_lengths) and this has to reflect the original
        # window. Zero for a short insert, whose segment is drawn exactly to the fragment's
        # edges (see generate_reads) — there is no literal headroom there to discard, only real
        # fragment/deletion. Nonzero for an ordinary read, whose segment includes the deletion
        # headroom drawn beyond the window.
        self.template_padding = max(0, len(reference_segment) - self.genomic_length)
    ...
    def _cigar_from_error_indels(self, error_indels):
        ...
        Walks the error list in reference-window order, emitting M/I/D ops and stopping once
        the query side reaches genomic_length (== run_read_length unless a 3' adapter tail
        occupies the rest of the read) — except a deletion anchored exactly at that point still
        gets emitted, since a D consumes no query bases. Run-length-encodes the result as it goes.
        """
        ...
        for ev in events:
            ...
            if m_emitted < m_count:
                # The read ended before reaching this event's anchor; it never happened
                # within this read.
                break
            if ev.error_type is Insertion:
                ...
                # A deletion consumes no query bases, so it still belongs in the CIGAR even
                # when the M's leading up to its anchor used up the last of the read.
                emit('D', ev.length)
                ref_pos += ev.length
    ...
    def _cigar_via_alignment(self):
        ...
        for template_base, query_base in zip(aligned_template, aligned_query):
            if query_used == self.genomic_length and (self.template_padding or self.is_reverse):
                # Everything past the read's last base is unused deletion headroom, not part of
                # its alignment — but only once the template actually carries literal padding.
                # A short insert's forward template is drawn to the fragment's exact edges (see
                # self.template_padding), so any trailing D there is a real deletion reaching the
                # fragment's end, not headroom, and has to stay in the CIGAR.
                #
                # Reverse reads keep the old behavior regardless: reverse-complementing puts this
                # same trailing zone at the read's *leading* edge, where query_right_open_gap_score
                # being free lets the aligner collapse an unrelated interior mismatch run into a
                # spurious gap here too, not just a genuine edge deletion — recovering that isn't
                # cheap (see test_short_insert_deletion_at_a_reverse_reads_three_prime_edge).
                break
            if template_base == '-':
                ops.append('I')
                query_used += 1
