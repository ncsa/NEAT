...
from ...common import ALLOWED_NUCL, open_input
from .ploid_functions import pick_ploids
from .options import Options

__all__ = ["parse_input_vcf"]

_LOG = logging.getLogger(__name__)


def retrieve_attribute_from_format(my_record: list, value, is_cancer=False):
...
def parse_input_vcf(vcf_path: str, ploidy: int, homozygous_frequency: float,
                    reference: SeqIO, options: Options, tumor_normal: bool = False) -> (dict, dict):
        ...
        for line in f:
            ...
            # Process the header row
                ...
                if 'FORMAT' in columns:
                    ...
                    if not sample_columns:
                        raise ValueError('Sample column missing in in input vcf.', 'error')
                else:
                ...
                if sample_columns:
                    ...
                    # If the code got here, we're dealing with a cancer sample
                        ...
                        _LOG.error(f'Tumor-Normal samples require '
                                   f'both a tumor and normal sample column in the VCF. {list(sample_columns)}')
                        raise ValueError
                    else:
            ...
            # Process the records rows
                ...
                if any([x for x in record[3] if x not in ALLOWED_NUCL]) or \
                    ...
                    mismatched += 1
                    _LOG.warning(f'Skipping variant because the ref or alt field contains invalid characters:'
                ...
                if has_format:
                    if "GT" in record[8].split(':'):
                        ...
                        genotype = retrieve_attribute_from_format(record, 'GT', False)
                        if tumor_normal:
                            # Same procedure as above, but with the tumor sample
                            tumor_sample_column = record[10]
                            genotype_tumor = retrieve_attribute_from_format(record, 'GT', True)
                    else:
                        ...
                        if tumor_normal:
                            ...
                            if genotype == genotype_tumor:
                                _LOG.warning(f'Skipping variant that already had a '
                ...
                if key not in ret_dict:
                    ...
                    if ret_dict[key][1] == genotype or ret_dict[key][2] == genotype_tumor:
                        ...
                        _LOG.warning(f'Skipping variant because multiple variants found at this location: {key}')
                        n_skipped += 1
                    else:
    ...
    _LOG.info(f'Found {len(ret_dict)} variants in input VCF.')
