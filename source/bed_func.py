...
def parse_bed(input_bed: str, chromosomes: list,
    ...
    if input_bed:
        ...
        try:
            for line in f:
                if not line.startswith(('@', '#', "\n")):
                    ...
                    try:
                        ...
                        print(f"ERROR: improperly formatted bed file line {line}")
            ...
            logging.error("Input bed files must be a valid bed files or a valid gzipped bed file. "
                          "Gzipped files must end in extension '.gz'")
            if debug:
                logging.error(e)
            will_exit(1)
                        ...
                        try:
                            ...
                            if len(mut_str) > 1:
                                if debug:
                                    ...
                                    logging.warning(f"Record with multiple mut_rates: {region}")
                                ...
                                if pos2 - pos1 < 0:
                                    ...
                                    if debug:
                                        print(f'Warning: Skipping invalid region (end - start < 0): {region}')
                                        logging.warning(f'Skipping: {region}')
                            if not mut_str and (not printed_warning or debug):
                                if debug:
                                    print(f"Warning: Mutation rate record(s) in bed {input_bed} with no mutation rate "
                                    ...
                                    logging.warning(f"Invalid mutation rate bed record: {key}, {ret_dict[key]}")
                            ...
                            logging.error("Malformed mutation bed file. Must be of the format "
                                          "'chromosome\tpos1\tpos2\tmut_rate=X.XX'.")
                            if debug:
                                logging.error(e)
